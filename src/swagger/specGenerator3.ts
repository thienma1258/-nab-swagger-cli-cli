...
import { Swagger } from '../model/swaggerModel';
...
export class SpecGenerator3 extends SpecGenerator {
  ...
  private buildServers() {
    ...
    const mockServerURL = basePath;
    const result :Swagger.Server[] =[{
      url:"{protocol}://"+mockServerURL,
    ...
    if (this.config.host){
      for(let i=0;i<this.config.host.length;i++){
        const url = `{protocol}://${this.config.host[i].url}${basePath}`
        result.push({
          url:url,
          description:this.config.host[i].description,
            variables:{
              protocol:{
                enum:['http','https'],
                default:'http'
